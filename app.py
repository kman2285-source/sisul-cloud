...
# 🔄 [수정 1] st.data_editor 강제 리프레시를 위한 버전 관리 세션 상태 초기화
...
with st.expander("⚙️ 고급 설정: 표 항목(열) 추가 및 삽입 / 이름 변경 / 순서 고정", expanded=False):
    ...
    with tab1:
        with st.form("add_column_form", clear_on_submit=True):
            ...
            # 🎯 엑셀처럼 원하는 위치에 열을 삽입할 수 있도록 선택창 추가
            ...
            if st.form_submit_button("➕ 항목 삽입/추가") and new_col_name:
                ...
                if new_col_name in df.columns or new_col_name == "NO": 
                    ...
                    with st.spinner("항목 삽입 중..."):
                        ...
                        settings_ref.update({"column_order": col_order})
                        st.session_state.table_version += 1  # 🔄 [수정 1] 컴포넌트 키 변경
                        st.cache_data.clear()
                        st.rerun()
...
if save_btn:
    editor_state = st.session_state.get(editor_key, {}) # 🎯 동적 키로 데이터 가져오기
    has_changes = False
...
if photo_col:
    ...
    # 🎯 [수정 2] 표의 맨 밑(마지막 행)부터 역순으로 불러오기
