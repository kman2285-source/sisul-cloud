...
if not (st.session_state.google_auth and st.session_state.pw_auth):
    ...
    # 두 인증을 모두 완료하기 전까지 본문 코드 실행을 강제로 멈춥니다.
    st.stop()
# ---------------------------------------------------------

# 🛡️ [단축키 방패] C키 팝업 창 방지 (로그인 완료된 사용자만 바인딩)
components.html(
    """
    <script>
...
# 💓 [해결방법 1] 세션 만료 방지 (3분마다 서버에 생존 신호 보내기)
components.html(
    """
    <script>
    ...
    }, 180000); // 3분(180,000ms)마다 백그라운드에서 자동 실행
    </script>
    """,
    height=0, width=0
)
...
# 🛡️ [해결방법 2] 안전한 단건 입력 팝업 폼 (데이터 증발 방지)
