...
# 🔄 [수정] st.data_editor 강제 리프레시를 위한 버전 관리 세션 상태 초기화
            ...
            if st.form_submit_button("➕ 항목 삽입/추가") and new_col_name:
                ...
                if new_col_name in df.columns or new_col_name == "NO": 
                    ...
                    with st.spinner("항목 삽입 중..."):
                        ...
                        settings_ref.update({"column_order": col_order})
                        st.session_state.table_version += 1  # 🔄 [수정] 에디터 컴포넌트 강제 리프레시
                        st.cache_data.clear()
                        st.rerun()
...
# 🎯 [수정] 동적 키 정의 (열 삽입/정렬 시 화면 강제 리마운트 효과)
...
# 4. 수동 일괄 저장 로직
if save_btn:
    editor_state = st.session_state.get(editor_key, {})  # 🎯 동적 키로 세션 탐색하도록 변경
    has_changes = False
